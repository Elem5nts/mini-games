...
A small collection of standalone, no-build web toys plus the **NMS Lookup**
reference tool. The repo root also holds a few self-contained game pages
(`bagger-aufraeumer.html`, `dino-tap.html`, `gaming-profile.html`) — open any
...
When Pages is enabled for this repo (Deploy from a branch, folder `/ (root)`),
...
All paths in the app are relative, so it works correctly under the `/<repo>/`
...
└─ docs/       # SCHEMA (item data contract)
```
